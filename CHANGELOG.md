...
- **FORGE-BUG-048: AJV packaging fix** — `ajv` added to `bundledDependencies`
  in `package.json`. Without bundling, npm deduplication could leave an empty
  `node_modules/ajv/` directory in packed global installs, causing
  `ERR_MODULE_NOT_FOUND` at session start. AJV is now shipped inside the
  tarball alongside the other bundled dependencies.
