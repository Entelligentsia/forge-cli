...
export function parseConfigArgs(args: string[]): ParseConfigResult {
	...
	return {
		error: `forge config: unknown subcommand "${sub}". Try: forge config show [--resolved] [--json] | dispatch [--pipeline=<name>] [--json]`,
	};
}
...
export async function runConfigShow(opts: ConfigArgs, cwd: string, write: WriteFn): Promise<number> {
...
export function runConfigDispatch(args: ConfigArgs, cwd: string, write: (line: string) => void): number {
	...
	const bugPhases: DispatchPhaseRow[] = BUG_PHASES.map((p) => traceOne(bugPipeline, p.role, p.personaNoun));
	...
	const fmt = (row: DispatchPhaseRow): string => {
		const req = row.requested ? `${row.requested.provider}:${row.requested.model}` : "(inherit pi current)";
		let find: string;
