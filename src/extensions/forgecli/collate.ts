...
export function parseCollateArgs(rawArgs: string, cwd: string): ParsedArgs {
...
export function composeKickoff(opts: ComposeKickoffOpts): string {
	...
	const sections: string[] = ["# /forge:collate", ""];
	...
	sections.push(
		...
		"",
		"1. Identify the active sprint via `forge_store_query` — do NOT raw-read `.forge/store/`.",
		"2. Follow the workflow Algorithm verbatim: collate store data into MASTER_INDEX.md, sprint INDEX.md, task INDEX.md files.",
		"3. Write all collated markdown files using the `write` tool.",
		"4. Honour Pack-06 Read/Write/Ask/Store discipline: store reads go via `forge_store_query`; in-conversation clarifications use `forge_ask_user`.",
	);
...
export function registerCollate(pi: ExtensionAPI, options: RegisterCollateOptions = {}): void {
	pi.registerCommand("forge:collate", {
		description:
			"Run the collator-agent workflow. " +
			...
			if (personas.length > 0) {
				try {
					...
					if (err instanceof PersonaSkillLoaderError) {
						ctx.ui.notify(
							`× forge:collate — persona '${personas[0]}' load failed (${err.code}): ${err.message}`,
							"error",
						);
						return;
					}
