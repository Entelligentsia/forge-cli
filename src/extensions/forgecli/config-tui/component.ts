...
import { writeRoutingConfig } from "../config/config-writer.js";
...
function persistLayerForAction(action: ConfigTuiAction): ConfigLayer | undefined {
	switch (action.kind) {
		...
		case "commit-override-name":
			return "project"; // L4 lives on project only
...
export class ConfigTuiComponent implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		if (this.state.confirmQuit) {
			...
			const screen = createScreen(view);
			contentLines = [...screen.render(this.state, width, theme), ...renderSaveBanner(this.state, width, theme)];
		}
	...
	handleInput(data: string): void {
		debugLog(
			`handleInput IN data="${hexEscape(data)}" focused=${this.focused} view=${getActiveView(this.state).kind}`,
		);
