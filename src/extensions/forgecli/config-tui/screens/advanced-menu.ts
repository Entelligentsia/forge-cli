...
import { cursor, accentBold, muted, dirtyMarker } from "../theme.js";
...
export class AdvancedMenuScreen implements Screen {
  render(state: ConfigTuiState, width: number, theme: Theme): string[] {
    ...
    // ── Menu items ─────────────────────────────────────────────────────
    const items = [
      "Override one persona's model",
      "Override one step's model",
      "Edit raw persona-models entries",
    ];
  ...
  handleInput(data: string, state: ConfigTuiState): InputResult {
    ...
    if (matchesKey(data, Key.enter)) {
      ...
      if (view.cursor === 1) {
        ...
        return { kind: "dispatch", action: { kind: "push-view", view: { kind: "overrides-list-pipelines", cursor: 0 } } };
      }
      if (view.cursor === 2) {
    ...
    // ── Shortcuts ──────────────────────────────────────────────────────
    if (matchesKey(data, "1")) return { kind: "dispatch", action: { kind: "push-view", view: { kind: "personas-list", cursor: 0 } } };
    if (matchesKey(data, "2")) return { kind: "dispatch", action: { kind: "push-view", view: { kind: "overrides-list-pipelines", cursor: 0 } } };
    if (matchesKey(data, "3")) return { kind: "dispatch", action: { kind: "push-view", view: { kind: "persona-picker", cursor: 0 } } };
