...
import { authBadge, muted, error as themedError, rule as themedRule, truncateLines } from "../theme.js";
...
export function authStatusLine(state: ConfigTuiState, theme: Theme): string {
	...
	const providers = [
		...new Set([...state.authenticatedProviders, ...state.availableModels.map((m) => m.provider)]),
	].sort();
...
export function resolvedSummary(state: ConfigTuiState, theme: Theme): string {
	...
	return muted(
		`  Resolved      ${total} persona-model assignment${total === 1 ? "" : "s"} · ${routable} routable`,
		theme,
	);
}
 ...
 */
export function filterWithSearch<T>(items: T[], query: string, getText: (item: T) => string): T[] {
