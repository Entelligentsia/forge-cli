...
export function listResolvedPersonas(state: ConfigTuiState): ResolvedPersonaEntry[] {
...
export function listPersonaPickerEntries(state: ConfigTuiState): PersonaPickerEntry[] {
	const assignmentsByName = new Map(listResolvedPersonas(state).map((p) => [p.persona, p] as const));
...
export function listPipelineOverrideSummaries(state: ConfigTuiState): PipelineOverrideSummary[] {
	...
	return names.map((pipeline) => {
		...
		const overrideCount = Object.values(phases).filter((p) => p["model-override"] !== undefined).length;
...
export function getPhaseOverride(
	...
	return state.buffer.project.pipelines?.[pipeline]?.phases?.[phaseRole]?.["model-override"];
}
 ...
 */
export function sourceToLabel(rawSource: ResolveSource, tier: Tier | undefined, isProject: boolean): string {
	switch (rawSource) {
		case "L4-inline":
		case "L4-name":
...
export function getPhaseTable(state: ConfigTuiState): PhaseTableRow[] {
	...
	return PHASE_PERSONA_TABLE.map((phase) => {
		...
		const isProject =
			result.source === "L2" ||
			result.source === "L4-inline" ||
			result.source === "L4-name" ||
			result.source === "L3";
		return {
			role: phase.role,
...
export function getTierAssignment(state: ConfigTuiState, tier: Tier): TierAssignment {
	const personas = TIER_PERSONAS[tier];
	const resolved = listResolvedPersonas(state);
	const resolvedMap = new Map(resolved.map((p) => [p.persona, p] as const));

	const entries = personas.map((p) => resolvedMap.get(p));
	...
	const allSame = entries.every((e) => e!.provider === first.provider && e!.model === first.model);
...
export function getScopedTierAssignment(state: ConfigTuiState, tier: Tier, scope: ConfigLayer): ScopedTierAssignment {
	const personas = TIER_PERSONAS[tier];
