...
export class DashboardController {
	...
	private handleCancelConfirm(data: string): void {
		if (data === "y" || matchesKey(data, Key.enter)) {
			if (this.state.cancelTargetId) {
				this.tree.requestCancel(this.state.cancelTargetId);
			}
	...
	private startCancel(): void {
		const node = this.tree.getNode(this.state.cursorId);
		if (!node) return;
		// Can cancel running nodes, or the root orchestrator session.
		if (node.status === "running" || node.status === "cancelling") {
			this.state.cancelTargetId = node.id;
		}
	}
...
export class DashboardComponent implements Component {
	...
	render(width: number): string[] {
		...
		// Layout: left panel 30%, right panel 70%, borders.
		const leftWidth = Math.max(30, Math.floor(width * 0.30));
		...
		const hints = " ↑↓ select · → expand · ← collapse · ⏎ focus · x stop · esc close";
	...
	private renderDetailPanel(node: OrchestratorNode, width: number): string[] {
		...
		lines.push(`${this.nodeGlyph(node)} ${bold(statusLabel)}${dim(modelPart)}`);

		// ── Metrics line ────────────────────────────────────────────────
		const metrics = this.formatMetrics(node);
		if (metrics) lines.push(dim(metrics));
		lines.push("");
		...
		if (node.kind === "orchestrator" && node.children.length > 0) {
			...
			lines.push(dim(bold(`Agents · ${children.length}`)));
			for (const child of children) {
				...
				lines.push(
					` ${cglyph} ${child.label}${dim(cmodel)}${dim(cmetricsPart)}`,
				);
			}
			lines.push("");
		}
		...
		if (node.promptPreview) {
			...
			lines.push(dim(`${expandIcon} Prompt · ${lineCount} lines · ⏎ expand`));
			if (this.controller.getState().promptExpanded) {
				for (const pline of node.promptPreview.split("\n").slice(0, 20)) {
					lines.push(dim(`  ${truncateToWidth(pline, width - 4)}`));
				}
				if (lineCount > 20) {
		...
		if (node.kind === "leaf" && node.tailBuffer.length > 0) {
			...
			lines.push(dim(`Activity · ${total} log line${total === 1 ? "" : "s"}`));
		...
		if (node.outcomePreview) {
			...
			for (const oline of node.outcomePreview.split("\n").slice(0, 8)) {
				lines.push(`  ${truncateToWidth(oline, width - 4)}`);
			}
	...
	private renderCancelConfirm(width: number, targetId: string): string[] {
		...
		const padW = Math.max(0, width - visibleWidth(prompt) - visibleWidth(actions) - 4);
		...
		lines.push(
			border("│") +
				" " +
				prompt +
				" ".repeat(Math.max(0, padW)) +
				" " +
				actions +
				" ".repeat(Math.max(0, width - 4 - visibleWidth(prompt) - visibleWidth(actions) - padW)) +
				border("│"),
		);
		lines.push(border("│") + " ".repeat(width - 2) + border("│"));
		lines.push(border("╰" + "─".repeat(width - 2) + "╯"));
