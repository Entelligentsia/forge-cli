...
export function composeKickoff(opts: ComposeKickoffOpts): string {
	...
	if (parsed.phase === 2) {
		...
		sections.push(
			...
			"5. **CRITICAL — snapshot after apply (forge#107 / forge-cli#30)**: After applying any approved enhancement edits (workflow body Step 8), MUST call `manage-versions add-snapshot` via bash to capture the edits into `.forge/archive/snap-N/`. Without this, the next `/forge:regenerate` overwrites the edits and they CANNOT be restored via replay (the entire durability story relies on a snapshot existing). Format:\n   ```sh\n   node \"$FORGE_ROOT/tools/manage-versions.cjs\" add-snapshot --source post-sprint:<SPRINT_ID> --enhanced-elements \"<comma-separated paths, e.g. .forge/personas/architect.md,.forge/skills/engineer-skills.md>\"\n   ```\n   `$FORGE_ROOT` is exported into the bash environment — see the orientation block above for its resolved value. After add-snapshot, verify success by checking `.forge/structure-versions.json`'s `currentSnapshot` advanced.",
		);
