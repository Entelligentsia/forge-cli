...
// (run-task, run-sprint, fix-bug) dispatch via runForgeSubagent directly
// and do NOT route through this handler.
//
// Iron Laws:
...
export function parseReviewPlanArgs(rawArgs: string, cwd: string): ParsedArgs {
...
export function composeKickoff(opts: ComposeKickoffOpts): string {
	...
	sections.push(
		...
		"",
		"1. Read the plan at `engineering/sprints/<SPRINT_ID>/<TASK_ID>/PLAN.md` (the source of truth).",
		"2. Query the store for the task and its sprint/feature context via `forge_store_query` — do NOT raw-read `.forge/store/`.",
		"3. Follow the workflow Algorithm verbatim: review the plan for correctness, completeness, and risk.",
		"4. Write `PLAN_REVIEW.md` and `PLAN-REVIEW-SUMMARY.json` to the task directory using the `write` tool.",
...
export function registerReviewPlan(pi: ExtensionAPI, options: RegisterReviewPlanOptions = {}): void {
	pi.registerCommand("forge:review-plan", {
		description:
			"Run the review-plan workflow for a Forge task. " +
			...
			try {
				...
				if (err instanceof WorkflowLoaderError) {
					if (err.code === "missing_file") {
						ctx.ui.notify(
							`× forge:review-plan — workflow not found at ${WORKFLOW_REL_PATH}; run /forge:init or /forge:regenerate first.`,
							"error",
						);
					} else {
			...
			if (personas.length > 0) {
				try {
					...
					if (err instanceof PersonaSkillLoaderError) {
						ctx.ui.notify(
							`× forge:review-plan — persona '${personas[0]}' load failed (${err.code}): ${err.message}`,
							"error",
						);
						return;
					}
