...
export function composeKickoff(opts: ComposeKickoffOpts): string {
	...
	sections.push(
		...
		"4. Write `PLAN_REVIEW.md` and `REVIEW-PLAN-SUMMARY.json` to the task directory using the `write` tool.",
			...
			try {
				...
				if (err instanceof WorkflowLoaderError) {
					if (err.code === "missing_file") {
						ctx.ui.notify(
							`× forge:review-plan — workflow not found at ${WORKFLOW_REL_PATH}; run /forge:init or /forge:rebuild first.`,
							"error",
						);
					} else {
