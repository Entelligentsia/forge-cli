...
export function checkRemit(emitted: Event[], node: NodeDef, loopItemId?: string): RemitCheckResult {
		...
		if (term.type === "success" && term.writes?.artifact) {
			...
			if (!pattern) {
				...
				if (term.writes.artifact.path !== resolved) {
					violations.push(
						`artifact path '${term.writes.artifact.path}' does not match declared pattern '${resolved}'`,
					);
				}
			}
		}
