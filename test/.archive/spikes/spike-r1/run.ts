...
function runAntiPatternGrep(): { pass: boolean; detail: string } {
	...
	const grep2 = spawnSync(
		"grep",
		["-rE", "--exclude=run.ts", "--exclude=run.js", "--exclude=RESULT.md", "JSON\\.stringify\\([^)]*chain", spikeDir],
		{ encoding: "utf-8" },
	);
...
function writeResultMd(opts: {
	...
	const modelDisplay = opts.runType === "OFFLINE MOCK" ? '"mock — does not discharge R1 gate"' : opts.modelId;
...
- Total wall-clock: ${opts.totalMs ?? "N/A"}ms`;
	...
	const gateStatus = {
		...
		"OFFLINE MOCK": "OFFLINE MOCK: R1 gate NOT discharged. T10 must run live follow-up.",
		DEFERRED: "DEFERRED (no API key): R1 gate NOT discharged. T10 must run live follow-up.",
	...
	const mockFlag = opts.runType === "OFFLINE MOCK" ? " [MOCK]" : "";
...
async function main(): Promise<void> {
	...
	assert("AC5 anti-pattern grep", grepResult.pass, grepResult.detail);
	...
	if (!hasApiKey) {
		...
		assert(
			...
			phase2TaskLiteral ? `chain[0].task = "${phase2TaskLiteral}"` : "chain args not captured",
		);
		...
		assert(
			...
			taskWithContext ? `taskWithContext = "${taskWithContext.slice(0, 80)}..."` : "taskWithContext not captured",
		);

		// AC3 — notify called (mock always does)
		assert("AC3 notify called after both phases", true, "[MOCK] notify would be called after Phase 2");
	...
	assert(
		...
		phase2Result !== undefined ? "tool_execution_end result recorded" : "tool_execution_end not captured for Phase 2",
	);
	...
	const assertionSummary = assertions.map((a) => `- [${a.pass ? "PASS" : "FAIL"}] ${a.name}: ${a.detail}`).join("\n");
