...
describe("tier-meta — structure consistency", () => {
  ...
  it("all 9 personas are accounted for across tiers (no duplicates, no gaps)", () => {
    ...
    const expected = new Set([
      "architect", "supervisor",       // heavy
      "engineer", "bug-fixer", "qa-engineer", "product-manager", // standard
      "collator", "librarian", "orchestrator", // light
    ]);
...
describe("tier-meta — PHASE_PERSONA_TABLE consistency", () => {
  it("has 8 phases matching CANONICAL_PHASES", () => {
    ...
    expect(roles).toEqual([
      "plan", "review-plan", "implement", "review-code",
      "validate", "approve", "writeback", "commit",
    ]);
  });
...
function makeState(overrides: Partial<Parameters<typeof initialState>[0]> = {}): ConfigTuiState {
  return initialState({
    ...
    project: null,
    cwd: "/home/x/proj",
    personaCatalogue: ["architect", "engineer", "supervisor", "bug-fixer", "qa-engineer",
      "product-manager", "collator", "librarian", "orchestrator"],
