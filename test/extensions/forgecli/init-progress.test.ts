...
describe("readInitProgress", () => {
	...
	it("T03: returns stale when lastPhase > 4 (12-phase legacy)", () => {
		mockFs.readFileSync.mockReturnValue(JSON.stringify({ lastPhase: 7, timestamp: "2026-01-01T00:00:00Z" }));
	...
	it("T05: returns valid for clean 4-phase checkpoint", () => {
		mockFs.readFileSync.mockReturnValue(JSON.stringify({ lastPhase: 2, timestamp: "2026-05-09T00:00:00Z" }));
	...
	it("returns valid for phase 1", () => {
		mockFs.readFileSync.mockReturnValue(JSON.stringify({ lastPhase: 1, timestamp: "2026-05-09T00:00:00Z" }));
...
describe("writeInitProgress", () => {
	...
	it("creates .forge/ directory before writing", () => {
		...
		expect(mockFs.mkdirSync).toHaveBeenCalledWith(path.join("/some/project", ".forge"), { recursive: true });
	});
...
describe("deleteInitProgress", () => {
	...
	it("deletes the progress file", () => {
		...
		expect(mockFs.unlinkSync).toHaveBeenCalledWith(path.join("/some/project", ".forge", "init-progress.json"));
	});
