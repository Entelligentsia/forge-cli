...
import {
  ForgeInputRouter,
  type RouterListener,
} from "../../../src/extensions/forgecli/input-router.js";
...
describe("ForgeInputRouter — registration", () => {
  it("calls each listener in registration order", () => {
    const calls: string[] = [];
    const a: RouterListener = (data) => { calls.push(`a:${data}`); return undefined; };
    const b: RouterListener = (data) => { calls.push(`b:${data}`); return undefined; };
  ...
  it("unsubscribe removes the listener", () => {
    ...
    const unsub = router.register(
      (data) => { calls.push(data); return undefined; },
      { name: "a" },
    );
  ...
  it("listener returning {consume: true} stops the chain", () => {
    const calls: string[] = [];
    router.register((data) => { calls.push(`a:${data}`); return { consume: true }; }, { name: "a" });
    router.register((data) => { calls.push(`b:${data}`); return undefined; }, { name: "b" });
  ...
  it("listener rewriting {data: y} passes the rewritten value to next", () => {
    const calls: string[] = [];
    router.register((data) => { calls.push(`a:${data}`); return { data: "Y" }; }, { name: "a" });
    router.register((data) => { calls.push(`b:${data}`); return undefined; }, { name: "b" });
    const result = router.dispatch("x");
...
describe("ForgeInputRouter — overlay awareness", () => {
  it("when no overlay active, listeners with skipWhenOverlayActive run normally", () => {
    const calls: string[] = [];
    router.register(
      (data) => { calls.push(`activator:${data}`); return { consume: true }; },
  ...
  it("when overlay is active, listeners WITHOUT skipWhenOverlayActive still run", () => {
    const calls: string[] = [];
    router.register((data) => { calls.push(`always:${data}`); return undefined; }, {
      name: "always-on",
      skipWhenOverlayActive: false,
    });
    router.register((data) => { calls.push(`skip:${data}`); return undefined; }, {
      name: "skip",
  ...
  it("default skipWhenOverlayActive is false (listeners run regardless)", () => {
    const calls: string[] = [];
    router.register((data) => { calls.push(data); return undefined; }, { name: "default" });
    router.pushOverlay();
    router.dispatch("y");
  ...
  it("after popOverlay restores, skipWhenOverlayActive listeners resume", () => {
    const calls: string[] = [];
    router.register(
      (data) => { calls.push(data); return { consume: true }; },
