...
import { MAX_TAIL_ENTRY_CHARS, OrchestratorTree, getOrchestratorTree } from "../../../src/extensions/forgecli/orchestrator-tree.js";
...
describe("OrchestratorTree", () => {
	...
	describe("tail buffer", () => {
		...
		// Storage contract: the model stores entries VERBATIM — one entry per
		// producer event, newlines and all. Display normalization (splitting,
		// wrapping, clamping, truncation) is owned entirely by the views
		// (toDisplayLines + wrapLine in the dashboard). The only model-side
		// transformation is a memory cap (capTailEntry), which is a storage
		// bound, not a display decision.
		it("appendTail stores multi-line entries verbatim — one entry per event", () => {
			tree.startNode("phase-1", { kind: "leaf" });
			const heredoc = "╭ $ bash cat << 'EOF' > /tmp/repro.mjs\n// Minimal reproduction\nEOF";
			tree.appendTail("phase-1", heredoc);
			expect(tree.getNode("phase-1")!.tailBuffer).toEqual([heredoc]);
		});

		it("appendTail caps entry size for memory, marking the cut", () => {
			tree.startNode("phase-1", { kind: "leaf" });
			const huge = "x".repeat(MAX_TAIL_ENTRY_CHARS + 500);
			tree.appendTail("phase-1", huge);
			const entry = tree.getNode("phase-1")!.tailBuffer[0]!;
			expect(entry.length).toBeLessThan(huge.length);
			expect(entry.startsWith("x".repeat(100))).toBe(true);
			expect(entry).toMatch(/…\(\+500c\)$/);
		});

		it("appendTail counts a warning once per multi-line entry", () => {
			tree.startNode("phase-1", { kind: "leaf" });
