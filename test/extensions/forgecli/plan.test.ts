// Unit tests for the /forge:plan native kickoff handler (FORGE-S20-T05).
//
// Conventions mirror enhance.test.ts: tmp-dir fixtures per test via
...
beforeEach(() => {
	tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "forgecli-plan-"));
});
...
const FULL_WORKFLOW = [
	...
	"## Persona reference",
	"",
	"You are the architect. See .forge/personas/architect.md for full identity.",
	"",
	"## Algorithm",
	"",
	"1. Load context via forge_store_query.",
	"2. Research, plan, write PLAN.md.",
].join("\n");
...
function makeStub(): StubResult {
	...
	return {
		...
		invoke: async (args: string) => {
			const cmd = registered.find((r) => r.name === "forge:plan");
			if (!cmd) throw new Error("forge:plan not registered");
...
describe("parsePlanArgs", () => {
	...
	it("@<absolute path> reads file", () => {
		...
		try {
			...
			const p = parsePlanArgs(`@${seedPath}`, "/some/other/cwd");
			expect(p.mode).toBe("file");
	...
	it("inline text → text mode, trimmed", () => {
		const p = parsePlanArgs("  FORGE-S20-T05  ", "/cwd");
		expect(p.mode).toBe("text");
		expect(p.taskRef).toBe("FORGE-S20-T05");
	});

	it("@<missing> throws", () => {
		expect(() => parsePlanArgs("@/no/such/file/exists.md", "/cwd")).toThrow();
	});
});

describe("extractPersonaNames", () => {
	it("returns names declared in deps.personas:", () => {
		expect(extractPersonaNames(FULL_WORKFLOW)).toEqual(["architect"]);
	});
...
describe("checkMaterialization", () => {
	it("happy path: all four markers present", () => {
		const res = checkMaterialization("/tmp/plan_task.md", FULL_WORKFLOW);
	...
	it("missing Store-Write Verification → reported", () => {
		...
		const res = checkMaterialization("/tmp/plan_task.md", md);
	...
	it("persona declared but not referenced in body → reported", () => {
		...
		const body = FULL_WORKFLOW.slice(fmEnd + 5).replace(/architect/g, "worker");
		const md = fm + body;
		const res = checkMaterialization("/tmp/plan_task.md", md);
		expect(res.ok).toBe(false);
...
describe("composeKickoff", () => {
	...
	it("text-mode appends task ref under Input — sourceLabel", () => {
		const out = composeKickoff({
			workflowMd: FULL_WORKFLOW,
			personaIdentity: "x",
			parsed: { mode: "text", taskRef: "FORGE-S20-T05", sourceLabel: "(seed from inline text)" },
		});
		expect(out).toContain("## Input — (seed from inline text)");
		expect(out).toContain("FORGE-S20-T05");
	});
...
describe("registerPlan — handler integration", () => {
	...
	it("free-form text argv: kickoff body contains the inline text under Input", async () => {
		...
		await stub.invoke("Plan task FORGE-S20-T05 quickly");

		expect(stub.pi.sendUserMessage).toHaveBeenCalledTimes(1);
		const [msg] = stub.pi.sendUserMessage.mock.calls[0];
		expect(msg as string).toContain("## Input — (seed from inline text)");
		expect(msg as string).toContain("Plan task FORGE-S20-T05 quickly");
	});
		...
		expect(
			stub.notifications.some(
				(n) =>
					...
					n.msg.includes(".forge/workflows/plan_task.md"),
			),
		).toBe(true);
	});
	...
	it("registers with description string and forge:plan name", async () => {
		const proj = scaffoldProject();
		const stub = makeStub();
		registerPlan(stub.pi as never, { cwd: proj });
		const cmd = stub.registered.find((r) => r.name === "forge:plan");
		expect(cmd).toBeDefined();
		expect(cmd!.description).toMatch(/plan-task workflow/i);
	});
});
