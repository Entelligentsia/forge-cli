...
const FIXTURE_SHA256 = "ab72c142eb6398a126dcdc8c5b9cb11bbff5576958ef84e0409dbbd2abe6fd14";
...
function loadFixture(): { ops: Op[]; raw: string } {
	...
	if (lines.length !== FIXTURE_LINES) {
		throw new Error(`fixture corruption: line count ${lines.length} !== ${FIXTURE_LINES}`);
	}
...
describe("store-friction regression (Mode A replay)", () => {
	it("locks the historical baseline; ratchets warn but do not fail", () => {
		...
		for (const [sub, baseline] of Object.entries(BASELINES)) {
			...
			expect(actual.ops, `pattern '${sub}' op count drift: ${actual.ops} vs baseline ${baseline.ops}`).toBe(
				baseline.ops,
			);
		...
		expect(elapsed, `wall-clock budget exceeded: ${elapsed.toFixed(0)}ms > ${WALLCLOCK_BUDGET_MS}ms`).toBeLessThan(
			WALLCLOCK_BUDGET_MS,
		);
	});
});
