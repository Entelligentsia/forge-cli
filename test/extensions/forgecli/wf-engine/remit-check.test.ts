...
describe("checkRemit", () => {
	...
	it("rejects two terminal events", () => {
		const events: Event[] = [makeEvent("started"), makeEvent("success"), makeEvent("failure", { reason: "oops" })];
	...
	it("accepts a failure with reason", () => {
		const events: Event[] = [makeEvent("started"), makeEvent("failure", { reason: "cannot-process" })];
		const result = checkRemit(events, makeNode());
		expect(result.ok).toBe(true);
	});

	it("rejects failure without reason", () => {
		const events: Event[] = [makeEvent("started"), makeEvent("failure")];
		const result = checkRemit(events, makeNode());
		expect(result.ok).toBe(false);
