...
const SCRIPT = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..", "scripts", "verify-publish.cjs");
...
function run(
	...
	const result = childProcess.spawnSync("node", [SCRIPT, "--root", rootDir, ...extraArgs], {
		encoding: "utf8",
		timeout: 15_000,
		env,
	});
...
describe("(d) --allow-non-latest — dist-tags.latest mismatch is warn not fail", () => {
	it("exits 0 and emits WARN to stderr (not FAIL)", () => {
		...
		const { exitCode, stderr } = run(npmDir, rootDir, ["--version", "0.4.0", "--allow-non-latest"]);
