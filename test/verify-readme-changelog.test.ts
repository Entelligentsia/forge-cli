...
function makeFixture(opts: { version: string; changelog: string; readme: string }): string {
...
function run(
	...
	const result = childProcess.spawnSync("node", [SCRIPT, "--root", dir, ...extraArgs], {
		encoding: "utf8",
		timeout: 10_000,
	});
	return {
...
describe("(d) no roadmap section in README", () => {
	it("exits 0 and emits a WARN to stderr", () => {
		tmpDir = makeFixture({
			...
			readme: ["# My Package", "", "Version 0.2.0 is released.", "", "## Links", "", "See changelog."].join("\n"),
		});
